...
function isDifferent(current, last) {
    if (!last) {
        return false;
    }
    return current.coverage.lines !== last.coverage.lines || current.eslint.warnings !== last.eslint.warnings || current.eslint.errors !== last.eslint.errors;
}
        ...
        lastCommit(packageJson.name, function (lastCommit) {
            ...
            parse(function (parsedReport) {
                ...
                if (SlackHookURL && SlackChannel) {
                    ...
                    var message = {
                        text: 'New results for <http://metrics:9002/#/project/' + packageJson.name + '|' + packageJson.name.toUpperCase() + '>, triggered by *' + parsedReport.dashboard.commit.author_name + '*',

...
function passesThreshold(current, last) {
    ...
    // Is the same data (don't report it!)
    if (current.coverage.lines !== last.coverage.lines || current.eslint.warnings !== last.eslint.warnings || current.eslint.errors !== last.eslint.errors) {
        return false;
    }

    // Make sure the coverage changes by at least 1%
    return Math.abs(current.coverage.lines - last.coverage.lines) >= 1;
}
